...
extern const uint32_t ifcNames_FromHardwareH2S = IfcNames_FromHardwareH2S;
extern const uint32_t ifcNames_IntoHardwareS2H = IfcNames_IntoHardwareS2H;
...
/************** Start of IntoHardwareWrapper CPP ***********/
#include "IntoHardware.h"
int IntoHardwaredisconnect_cb (struct PortalInternal *p) {
    (static_cast<IntoHardwareWrapper *>(p->parent))->disconnect();
    return 0;
};
int IntoHardwarehardware_input_cb (  struct PortalInternal *p, const int32_t v ) {
    (static_cast<IntoHardwareWrapper *>(p->parent))->hardware_input ( v);
    return 0;
};
IntoHardwareCb IntoHardware_cbTable = {
    IntoHardwaredisconnect_cb,
    IntoHardwarehardware_input_cb,
};

/************** Start of FromHardwareWrapper CPP ***********/
#include "FromHardware.h"
int FromHardwaredisconnect_cb (struct PortalInternal *p) {
    (static_cast<FromHardwareWrapper *>(p->parent))->disconnect();
    return 0;
};
int FromHardwarehardware_output_cb (  struct PortalInternal *p, const int32_t v ) {
    (static_cast<FromHardwareWrapper *>(p->parent))->hardware_output ( v);
    return 0;
};
FromHardwareCb FromHardware_cbTable = {
    FromHardwaredisconnect_cb,
    FromHardwarehardware_output_cb,
};
